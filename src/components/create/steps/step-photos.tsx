...
export function StepPhotos({ draft, patch }: StepProps) {
  ...
  return (
    <div>
      ...
      <ul className={cn("grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4", draft.photos.length > 0 && "mt-3")}>
        {draft.photos.length < MAX_PHOTOS && (
          <li
            ...
            className={cn(
              "flex aspect-[4/3] cursor-pointer flex-col items-center justify-center gap-1.5 rounded-lg border-2 border-dashed p-3 text-center transition-colors",
              dragging ? "border-accent bg-brand-50/70" : "border-border bg-card hover:border-accent/50 hover:bg-secondary/50",
            )}
          >
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-secondary text-muted-foreground">
              <UploadCloud className="h-4 w-4" />
            </span>
            <p className="text-[12.5px] font-medium leading-tight">Ավելացնել լուսանկար</p>
            <p className="text-[11px] leading-tight text-muted-foreground">JPG կամ PNG</p>
            <input
              ref={inputRef}
              type="file"
              accept="image/*"
